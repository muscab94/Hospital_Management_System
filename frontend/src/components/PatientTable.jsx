...
import { Search, Plus, Eye, Edit, Phone, MapPin } from "lucide-react";
...
function PatientTable() {
  ...
  const handleSearch = async (term) => {
    setSearchTerm(term);
    if (!term.trim()) {
      // if empty, fetch all patients again
      const token = localStorage.getItem("token");
      const res = await axios.get("http://localhost:5000/api/patients", {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        {user?.role === "admin" && (
          <button
            ...
            rounded-lg shadow hover:bg-gray-800"
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <div className="p-4 border-b">
          ...
          <p className="text-sm text-gray-500">All registered patients in the system</p>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {patients.map((patient) => (
              <tr key={patient._id || patient.id} className="border-t hover:bg-gray-50 text-sm">
                <td className="p-4">
                ...
                <td className="p-4">
                  <span className="px-2 py-1 text-xs rounded-full border">{patient.bloodGroup}</span>
                </td>
                <td className="p-4">
                  <span
                    className={`px-3 py-1 text-xs rounded-full ${
                      patient.isActive ? "bg-gray-900 text-white" : "bg-gray-200 text-gray-600"
                    }`}
                  >
