  ...
  const menuItems = [
    {
      ...
      roles: ["admin", "doctor", "receptionist"], // all staff
    },
    {
      to: "/Dashboard/patients",
      label: "Patients",
      icon: <Users className="w-5 h-5" />,
      roles: ["admin", "doctor", "receptionist"], // all except maybe nurse
    },
    {
    ...
    {
      to: "/Dashboard/contactList",
      label: "Contact List",
      icon: <i className="fa-solid fa-address-book"></i>,
      roles: ["admin", "doctor", "receptionist"], // everyone
    },
  ];
  ...
  return (
    <div className="min-h-screen w-64 fixed flex flex-col bg-blue-600 text-white shadow-lg">
      ...
      <div className="flex-1 bg-blue-700 py-10 px-3 space-y-3">
        {menuItems
          .filter((item) => item.roles.includes(user.role)) // filter by role
          .map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
